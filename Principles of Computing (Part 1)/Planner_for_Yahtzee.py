...
def score(hand):
    ...
    """
    return max([hand.count(value)*value for value in hand])
...
def expected_value(held_dice, num_die_sides, num_free_dice):
    ...
    return reduce(lambda res, x: res + score(list(held_dice) + list(x)),
                  rolls, 0.0) / len(rolls)
...
def gen_all_holds(hand):
    ...
    for idx in range(1 << len(hand)):
        result.append(tuple([hand[jdx] for jdx in range(len(hand)) if (idx & (1 << jdx))]))
    return set(result)
