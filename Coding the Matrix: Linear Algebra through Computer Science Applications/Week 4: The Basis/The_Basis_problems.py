...
indep_vec_1 = Vec({1, 2, 3, 4}, {1:1, 2:1})
indep_vec_2 = Vec({1, 2, 3, 4}, {2:1, 3:1})
indep_vec_3 = Vec({1, 2, 3, 4}, {3:1, 4:1})
indep_vec_4 = Vec({1, 2, 3, 4}, {4:1, 1:1})
...
def is_superfluous(S, v):
    ...
    L = list(S.copy())
    if len(L) == 1:
        return True if L[0].f == {} else False 
    bVec = L.pop(L.index(v)) 
    bMat = coldict2mat(L)
    residual = bVec - bMat * solve(bMat,bVec)
    return True if residual.is_almost_zero() else False
...
def exchange(S, A, z):
    ...
    L.append(z)
    for i in L:
        if i not in A and is_superfluous(L, i):
            return i
    return
