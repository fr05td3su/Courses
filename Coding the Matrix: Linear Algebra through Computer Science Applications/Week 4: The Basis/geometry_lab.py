...
def identity(labels = {'x','y','u'}):
    ...
    True
    '''
    return Mat((labels, labels), {(item, item): 1 for item in labels})
...
def reflect_about(x1, y1, x2, y2):
    ...
    True
    '''
    return translation(x2, y2) * rotation(math.atan2(y2 - y1, x2 - x1)) * reflect_x() * rotation(-math.atan2(y2 - y1, x2 - x1)) * translation(-x2, -y2)
