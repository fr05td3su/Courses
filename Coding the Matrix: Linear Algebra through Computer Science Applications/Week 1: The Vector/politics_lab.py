...
def create_voting_dict(strlist):
    ...
    """
    return {i.split()[0]: [int(x) for x in i.split()[3:]] for i in strlist} 
...
def most_similar(sen, voting_dict):
    ...
    values = {policy_compare(sen, x, voting_dict):x for x in voting_dict.keys() if x != sen}
...
voting_record_dump109 = create_voting_dict(list(f))

most_like_chafee    = most_similar('Chafee', voting_record_dump109)
least_like_santorum = least_similar('Santorum', voting_record_dump109) 
