...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return reduce(operator.and_, [getitem(A, k) == getitem(B, k) for k in product(A.D[0], A.D[1])])  
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {k: getitem(A, k) + getitem(B, k) for k in product(A.D[0], A.D[1])})
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {col: sum([v[row]*getitem(M, (row, col)) for row in M.D[0]]) for col in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], {row: sum([v[col]*getitem(M, (row, col)) for col in M.D[1]]) for row in M.D[0]}) 
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(i,j): sum([getitem(A,(i,x)) * getitem(B,(x,j)) for x in A.D[1]]) for i in A.D[0] for j in B.D[1]})
