...
first_five_squares = { x*x for x in range(1,6)}



## 6: (Task 6) Powers-of-2 Set Comprehension
first_five_pows_two = {2**x for x in range(0,5)}
...
#{x*y for x in {1,2,3} for y in {4,5,99}}
...
S_intersect_T = { x%(y+1) for x in S for y in T if x == y }
...
representation_dict = {x*base**2+y*base+z:(x, y, z) for x in digits for y in digits for z in digits if x < base**3 and y < base**2 and z < base}
